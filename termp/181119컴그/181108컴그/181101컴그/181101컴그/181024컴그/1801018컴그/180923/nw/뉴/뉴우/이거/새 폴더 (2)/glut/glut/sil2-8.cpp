...
public:
	...
	Test(GLVector3List vector3List, bool state, PolygonKind polygonKind) {
		...
		if (this->polygonNum == PKCi || this->polygonNum == PKTCi) {
...
void SetPolygon(GLVectorBasic datumPoint, PolygonKind polygonKind) {
	...
	if (poolingNum < objectNumLimit) {
		...
		if (polygonKind == PKRe) {
			...
			test[poolingNum] = Test(vector3List[poolingNum], true, polygonKind);
			...
			if (true) {//애니메이션 쓸건지에 대한 플레그
				...
				if (!IsTimerWorking()) {
					...
					timerActivate = true;
					glutTimerFunc(speed, TimerFunction, speed);
				}
				printf("%dppol\n", poolingNum);
			}
		...
		else if (polygonKind == PKCi) {
			vector3List[poolingNum].RemoveAll();
			size = 10;


			GLVectorBasic glvector3[5] = {
				GLVectorBasic(datumPoint.x , datumPoint.y, datumPoint.z),
				GLVectorBasic(datumPoint.x , datumPoint.y + size / 2, datumPoint.z),
				GLVectorBasic(datumPoint.x + size / 2, datumPoint.y , datumPoint.z),
				GLVectorBasic(datumPoint.x - size / 2, datumPoint.y , datumPoint.z),
			...
			if (random == 2 || random == 3) {
				...
				test[poolingNum].specialFlag = true;
			...
			test[poolingNum].tornadoDegreeMax = 1080;

			int pM = PlusOrMinus();

			for (int i = 0; i < test[poolingNum].tornadoDegreeMax; ++i) {

				test[poolingNum].tornadoPoints[i] = GLVectorBasic(test[poolingNum].radius* sin((i * PI) / 180.0) * pM + test[poolingNum].glVector3[0].x, test[poolingNum].radius* cos((i * PI) / 180.0) * pM + test[poolingNum].glVector3[0].y, test[poolingNum].glVector3[0].z);

				test[poolingNum].radius += 0.05;
...
void TimerFunction(int value) {
	...
	for (int i = 0; i < objectNumLimit; ++i) {
		...
		if (test[i].activate && timerFlag[i] == true) {
			...
			if (test[i].polygonNum == PKRe) {
			...
			else if (test[i].polygonNum == PKCi) {

				if (!test[i].specialFlag) {
					if (test[i].aniStack >= 8) {
						test[i].aniStack = 0;

						test[i].radius = test[i].originalRadius;
					}
					if (test[i].aniStack < 4) {
				...
				else if (test[i].specialFlag) {

					if (test[i].aniStack >= 600) {

					}


					else if (test[i].aniStack < 600) {
						...
						test[i].rGB[0].x = (rand() % 10) / 10.0;
						test[i].rGB[0].y = (rand() % 10) / 10.0;
						test[i].rGB[0].z = (rand() % 10) / 10.0;
			...
			else if (test[i].polygonNum == PKTCi) {

				if (!test[i].processingDirection) {
					if (test[i].aniStack >= test[i].tornadoDegreeMax) {

						test[i].processingDirection = true;
					}

					else if (test[i].aniStack < test[i].tornadoDegreeMax) {

						test[i].aniStack++;

						test[i].tornadoDegree++;


					}

				}
				else if (test[i].processingDirection) {

					if (test[i].aniStack <= 0) {

						test[i].processingDirection = false;
					}

					else if (test[i].aniStack > 0) {
...
GLvoid drawScene(GLvoid) {
	...
	for (int i = 0; i < objectNumLimit; ++i) {
		...
		if (test[i].activate) {
			...
			if (test[i].polygonNum == (int)PKRe) {
			...
			else if (test[i].polygonNum == (int)PKCi) {
				...
				for (int j = 0; j < 360; ++j) {
					...
					glVertex3d(test[i].radius* sin((j * PI) / 180.0) + test[i].glVector3[0].x, test[i].radius* cos((j * PI) / 180.0) + test[i].glVector3[0].y, test[i].glVector3[0].z);
			...
			else if (test[i].polygonNum == (int)PKTCi) {
				...
				for (int j = 0; j < test[i].tornadoDegree; ++j) {


					glVertex3d(test[i].tornadoPoints[j].x, test[i].tornadoPoints[j].y, test[i].tornadoPoints[j].z);
