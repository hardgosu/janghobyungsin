...
bool transformAndRestoration[objectNumLimit];
int transformAndRestorationStack[objectNumLimit];
		...
		if (rects[i].activate && timerFlag[i] == true) {
			...
			if (rects[i].polygonNum == PKRe) {
				...
				if (!rects[i].selected) {
					...
					if (rects[i].glVector3[2].y <= screenRBY) {


						while (rects[i].glVector3[2].y <= screenRBY) {
							rects[i].glVector3[3].y += 0.1;
							rects[i].glVector3[2].y += 0.1;
							rects[i].glVector3[1].y += 0.1;
							rects[i].glVector3[0].y += 0.1;
					...
					if (rects[i].glVector3[2].x >= screenRBX) {

						while (rects[i].glVector3[2].x >= screenRBX) {
							rects[i].glVector3[3].x -= 0.1;
							rects[i].glVector3[2].x -= 0.1;
							rects[i].glVector3[1].x -= 0.1;
							rects[i].glVector3[0].x -= 0.1;
					...
					if (rects[i].counterClock) {
						...
						if (rects[i].toDown && rects[i].glVector3[2].y >= screenRBY) {
						...
						if (rects[i].toRight && rects[i].glVector3[2].x <= screenRBX) {
				...
				if (rects[i].transformTriangle == true) {

					GLdouble increment = 5.0;




					if (rects[i].transformTriangleAniStack < 3) {

						rects[i].rGB[0] = GLVectorBasic((rand() % 10) / 10.0, (rand() % 10) / 10.0, (rand() % 10) / 10.0);

						rects[i].transformTriangleAniStack++;


						//grow bigger
						rects[i].glVector3[0].x -= increment;
						rects[i].glVector3[0].y += increment;

						rects[i].glVector3[1].x += increment;
						rects[i].glVector3[1].y += increment;

						rects[i].glVector3[2].x += increment;
						rects[i].glVector3[2].y -= increment;

						rects[i].glVector3[3].x -= increment;
						rects[i].glVector3[3].y -= increment;
						...
						rects[i].glVector3[0].x += ((rects[i].width + (rects[i].transformTriangleAniStack * increment* 2.0) ) / 3.0);
						if (rects[i].glVector3[0].x > rects[i].glVector3[3].x + (rects[i].width + (rects[i].transformTriangleAniStack * increment * 2))/ 2.0) {
							rects[i].glVector3[0].x = rects[i].glVector3[3].x + (rects[i].width + (rects[i].transformTriangleAniStack * increment * 2)) / 2.0;
						}



						rects[i].glVector3[1].x -= ((rects[i].width + (rects[i].transformTriangleAniStack * increment * 2.0)) / 3.0);
						if (rects[i].glVector3[1].x < rects[i].glVector3[2].x - (rects[i].width + (rects[i].transformTriangleAniStack * increment * 2)) / 2.0) {
							rects[i].glVector3[1].x = rects[i].glVector3[2].x - (rects[i].width + (rects[i].transformTriangleAniStack * increment * 2)) / 2.0;
						}
					...
					else if (rects[i].transformTriangleAniStack < 6) {
						rects[i].rGB[0] = GLVectorBasic((rand() % 10) / 10.0, (rand() % 10) / 10.0, (rand() % 10) / 10.0);
						rects[i].transformTriangleAniStack++;
						//grow bigger
						rects[i].glVector3[0].x += increment;
						...
						rects[i].glVector3[0].x -= ((rects[i].width + ((rects[i].transformTriangleAniStack% 4) * increment)) / 3.0);
						if (rects[i].glVector3[0].x < rects[i].glVector3[3].x) {
							rects[i].glVector3[0].x = rects[i].glVector3[3].x;
						}


						rects[i].glVector3[1].x += ((rects[i].width + ((rects[i].transformTriangleAniStack % 4) * increment)) / 3.0);
						if (rects[i].glVector3[1].x > rects[i].glVector3[2].x) {
							rects[i].glVector3[1].x = rects[i].glVector3[2].x;
						}
						...
						rects[i].rGB[0] = rects[i].rGBOriginal[0];
						rects[i].glVector3[0].x = rects[i].glVector3[3].x;
						rects[i].glVector3[1].x = rects[i].glVector3[2].x;

						rects[i].transformTriangleAniStack = 0;
						rects[i].transformTriangle = false;
			...
			if (bigRects[i].polygonNum == (int)PKBRe) {
				...
				for (int j = 0; j < objectNumLimit; ++j) {
					...
					if ((rects[j].Available() && rects[j].polygonNum == (int)PKRe) && checkCollision.CheckRectRect(Rect(rects[j].glVector3[0].x, rects[j].glVector3[0].y, rects[j].width, rects[j].height), Rect(bigRects[0].glVector3[0].x, bigRects[0].glVector3[0].y, bigRects[0].width, bigRects[0].height))) {
						...
						if (rects[j].transformTriangle == false) {
							rects[j].transformTriangle = true;
						}
						//rects[j].Destroy();
